...
class FaultsAlertsDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function (ProductFault $fault) {
            ...
            ->addColumn('action', function (ProductFault $fault) {
                return '<button class="btn btn-sm btn-success mark-reviewed-btn" data-id="' . $fault->id . '" title="Marcar como revisada">
                    <i class="fas fa-check"></i>
                </button>';
            })
            ->rawColumns(['image', 'status', 'stock_at_detection', 'action'])
            ->setRowId('id');
    }
    ...
    {
        return $this->builder()
            ->setTableId('faults-alerts-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('status')->title('Estado')->addClass('text-center')->searchable(false)->orderable(false),
            Column::computed('action')->title('Acción')->addClass('text-center')->searchable(false)->orderable(false)->width(80),
        ];
    }
}
